...
Writes preview/cyber-range-preview.html — one HTML file with every screen in it,
...
SCREENS = [
    ...
    ("register", "Register", "Name, year of study, username, password. No email is collected."),
    ("login", "Sign in", "Three attempts before lockout. Recovery is staff-issued, not emailed."),
    ...
    ("admin_user", "Account detail", "Unlock, email a reset link, and (admins only) set the role."),
    ("admin_sessions", "Sessions", "Close a stuck room and release its machine."),
...
def inline_assets(html: str) -> str:
    """Embed the tile SVGs and neutralise everything that would need a server."""
    for svg in (ROOT / "app" / "static" / "img").glob("*.svg"):
        html = html.replace(f"/static/img/{svg.name}", data_uri(svg))

    html = re.sub(r'<link rel="stylesheet"[^>]*>', "", html)
    html = re.sub(r"<script[^>]*></script>", "", html)
    html = re.sub(r'href="/[^"]*"', 'href="#" data-inert="1"', html)
    html = re.sub(r"<form ", '<form onsubmit="return false" ', html)
    # The clock is normally driven by room.js; show a plausible reading instead.
...
def build():
    ...
    out_file = out_dir / "cyber-range-preview.html"
...
<title>Cyber Range — interface preview</title>
